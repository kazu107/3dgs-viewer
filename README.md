...
1. 「ファイルを選択」またはウィンドウにファイルをドラッグ&ドロップ → その場でプレビュー
2. フライカメラで移動して見せたい構図を決め、「**現在の視点を記録**」をクリック(未記録ならアップロード時の視点が使われます)
3. 設定パネルで初期移動速度を調整(現在の速度がそのまま保存されます)
4. シーン名・説明を入力して「**R2 にアップロード**」

ファイル本体のアップロードと `scenes.json` への登録(開始位置・移動速度・LoD設定込み)まで自動で行われ、公開サイトのシーン一覧に即反映されます。
