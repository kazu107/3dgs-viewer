...
- **スマホAR(マーカー固定)**: 印刷したマーカーにシーンを固定してカメラ越しに表示([AR.js](https://github.com/AR-js-org/AR.js) + Spark)
...
2. 複数ファイルを追加した場合の扱いを選べます:
   - **合成(同時表示)**: 複数のスキャンを1つのワールドに合成。各レイヤーの位置(X/Y/Z)・向き・倍率を数値入力でその場で調整でき、プレビューに即反映されます。公開後は画面下部のバーでレイヤーの表示/非表示を切替
   - **切替(3DGS⇔点群)**: 同じシーンの別表現として登録し、カメラ位置を保ったまま切り替え
3. フライカメラで移動して見せたい構図を決め、「**+ 現在の視点を追加**」をクリック。複数登録でき、名前も付けられます(最初の視点が開始位置。未登録ならアップロード時の視点)
   - **COLMAPのカメラ位置から選ぶことも可能**: 「**images.txt を読み込む**」(またはimages.txtをドラッグ&ドロップ)→ ◀ ▶ で撮影カメラの位置を巡回 → 「視点に追加」。RealityScanの ALIGNMENT → Export → Registration → Colmap(ASCII)で出力したimages.txtに対応。シーンのX軸180°回転は自動で合わせます
4. 「**全体の配置**」でシーン全体の位置・回転(XYZ)・倍率を調整できます(傾いたスキャンの水平出しや、大きさ・向きの補正)。調整した状態がそのまま保存・表示されます
5. 設定パネルで初期移動速度を調整(現在の速度がそのまま保存されます)
6. シーン名・説明を入力して「**R2 にアップロード**」
...
| `layers` | **合成ワールド**: 複数ファイルを同時表示して1つのワールドに合成。各レイヤーに `key` と `transform`(`position` / `headingDeg` / `scale` / `rotationDeg`)を指定。画面下部のバーでレイヤーごとの表示/非表示を切替 |
| `world` | **シーン全体の配置**。`position` / `rotationDeg`(XYZ度)/ `scale` をシーン全体(全レイヤー)に適用。アップロードスタジオの「全体の配置」で調整した値が保存される |
...
印刷したマーカーの上にシーンを固定して、スマホのカメラ越しに表示します。WebXRを使わず[AR.js](https://github.com/AR-js-org/AR.js)(カメラ映像+画像認識)で動くため、**iOS Safari・Android Chrome 両対応**(アプリ不要)。

### 使い方

1. 通常ビューアでシーンを開き、上部の **ARアイコン**(六角形)をタップ → ARビューア(`/ar.html`)が開きます。QRコードに `https://<アプリ>/ar.html#scene=<シーンID>` を入れておけば、読み取るだけでそのシーンのARが起動します
2. マーカーの種類を選び、「**マーカーを表示 / 印刷**」で表示 → 印刷(または別画面に表示)して平らな面に置く
   - **Hiro マーカー**(標準・おすすめ): 追従が安定。公式画像リンクから印刷すると確実です
   - **バーコード(QR風マトリクス)マーカー**: [印刷用マーカー集](https://github.com/nicolocarpignoli/artoolkit-barcode-markers-collection)から番号を選んで印刷
3. 「**ARを開始**」→ カメラを許可 → マーカーにカメラを向けるとシーンが出現します
4. 実行中は「大きさ」「高さ」スライダーでシーンの見え方を調整できます

> **QRコードそのものは追従できません**。QRは模様が単純すぎて画像トラッカーが特徴点を拾えないため、上記の専用マーカーを使います。QRは「ARページのURLを開く」launcherとして併用するのが定番です。

### 制約(重要)

- **HTTPS必須**。カメラは `https://` でのみ使えます(Herokuは自動でHTTPS)。ローカルは `localhost` のみ可
- AR.jsは**SLAMではない**ため、シーンはマーカーがカメラに映っている間だけ固定されます。マーカーが画面外に出ると追従が切れます → 「机の上のマーカーに置いて眺める」用途向き。広く歩き回る用途には不向きです
- 小さいマーカーに大きいシーンを固定すると位置がブレやすくなります(なるべく大きく印刷)
- モバイルGPU負荷を抑えるため、AR表示は軽量なシーン(〜数十万スプラット)推奨
